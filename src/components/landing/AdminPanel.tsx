...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center overflow-x-hidden w-full">
      {/* Background */}
      <div className="absolute inset-0 hero-gradient">
        ...
        {/* Gradient Orbs - overflow-x-hidden ilə kənarlara çıxsa da scroll yaranmayacaq */}
      ...
      {/* Floating Product Cards - Sağ tərəf (Boş qalmışdı, nümunə məzmun əlavə etdim) */}
      <motion.div
        initial={{ opacity: 0, x: 100 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.8, duration: 0.8 }}
        className="hidden lg:block absolute right-8 xl:right-20 top-1/4 z-20"
      >
        <div className="relative">
          <motion.div
            animate={{ y: [0, -15, 0] }}
            transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
            className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 shadow-2xl w-64"
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-white/90 font-semibold text-sm">Sürətli Çatdırılma</p>
                <p className="text-white/60 text-xs">24 saat ərzində</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="px-2 py-1 bg-secondary/20 text-secondary text-xs rounded-full">Pulsuz</span>
              <span className="text-white/60 text-xs">Bakı daxilində</span>
            </div>
          </motion.div>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="hidden lg:block absolute left-8 xl:left-20 bottom-1/4 z-20"
      >
        <div className="relative">
          <motion.div
            animate={{ y: [0, 15, 0] }}
